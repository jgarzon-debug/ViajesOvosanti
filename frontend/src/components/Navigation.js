...
export default function Navigation() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#E2E8F0] shadow-lg z-[10000]" data-testid="bottom-navigation">
      <div className="w-full mx-auto flex justify-start pl-2">
        <div className="flex gap-2">
          <button
            onClick={() => navigate("/")}
            className={`flex flex-col items-center justify-center h-16 px-4 ${
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center h-16 px-4 ${
              isActive("/dashboard") ? "text-[#1E8E3E]" : "text-[#4B5563]"
            } active:scale-[0.95]`}
